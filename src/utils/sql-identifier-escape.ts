 ...
 * @throws SchemactError if identifier contains dangerous characters
 */
export function escapeSqlIdentifier(
  ...
  if (identifier.length === 0) {
    throw new SchemactError(`Invalid ${type}: cannot be empty`);
  }
...
export function escapeSqlStringLiteral(value: string): string {
  if (typeof value !== 'string') {
    throw new SchemactError('Value must be a string');
  }
