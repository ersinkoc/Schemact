...
import { SchemactError } from '../ast/types.js';
 ...
 * @throws SchemactError if name is invalid
 */
export function validateMigrationName(name: string): ValidationResult {
  if (!name || typeof name !== 'string') {
    throw new SchemactError('Migration name must be a non-empty string');
  }
  ...
  if (decoded.length === 0) {
    throw new SchemactError('Migration name cannot be empty');
  }
 ...
 * @throws SchemactError if path validation fails
 */
 ...
 * @throws SchemactError if path is outside allowed directory
 */
