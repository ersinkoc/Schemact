...
import { DbAdapter, SchemactError, SchemactConfig, MigrationMetricEvent } from '../ast/types.js';
...
export interface RunnerOptions {
  ...
  ledgerPath?: string;
  config?: SchemactConfig; // FIX CRITICAL-1: Add config for file size validation
}
...
export class MigrationRunner {
  ...
  private config?: SchemactConfig; // FIX CRITICAL-1: Store config
    ...
    try {
      ...
      if (migrationsToRecord.length > 0) {
        try {
          ...
          const enhancedError = new SchemactError(errorMessage);
   ...
   *
   * @throws {SchemactError} If migrations directory doesn't exist
