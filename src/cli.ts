...
/**
 * CLI: Command-line interface for Schemact
    ...
    try {
      switch (this.command) {
        ...
        default:
          ...
          console.log(`Run ${c.cyan('schemact help')} for usage information.`);
          process.exit(1);
      }
    } catch (error) {
...
`;
    ...
    try {
      ...
      console.log(c.warning('Config file already exists: schemact.config.js'));
    } catch {
      await writeFile(configPath, configContent, 'utf-8');
      console.log(c.success(`Created config file: ${c.dim('schemact.config.js')}`));
    }

    console.log();
    console.log(c.bold('Next steps:'));
    console.log(`  1. Edit ${c.cyan('schemact.config.js')} and configure your database adapter`);
    console.log(`  2. Run ${c.cyan('schemact create <name>')} to create your first migration`);
  ...
  private async create(): Promise<void> {
    ...
    if (!name) {
      throw new SchemactError('Migration name is required. Usage: schemact create <name>');
    }
  ...
  private showHelp(): void {
    console.log(c.bold('Schemact - Zero-Dependency Database Schema Management'));
    console.log();
    console.log(c.bold('Usage:'));
    console.log(`  ${c.cyan('schemact')} ${c.yellow('<command>')} ${c.dim('[options]')}`);
    console.log();
    console.log(c.bold('Commands:'));
    console.log(`  ${c.cyan('init')}              Initialize a new Schemact project`);
    ...
    console.log(`  ${c.dim('$')} schemact init`);
    console.log(`  ${c.dim('$')} schemact create add_users_table`);
    console.log(`  ${c.dim('$')} schemact up`);
    console.log(`  ${c.dim('$')} schemact up --database mysql`);
    console.log(`  ${c.dim('$')} schemact status -d sqlite`);
    console.log(`  ${c.dim('$')} schemact pull public`);
    console.log();
   ...
   */
  private async loadConfig(): Promise<SchemactConfig> {
    const configPath = resolve(process.cwd(), 'schemact.config.js');
    ...
    try {
      ...
      throw new SchemactError(
        `Config file not found. Run ${c.cyan('schemact init')} to create one.`
      );
    }
    ...
    try {
      ...
      throw new SchemactError(`Failed to load config file: ${(error as Error).message}`);
    }
  }
   ...
   */
  private validateConfig(config: SchemactConfig): void {
    if (!config.adapter) {
      throw new SchemactError(
        'Database adapter not configured. Please edit schemact.config.js and provide an adapter.'
      );
    }
  }
