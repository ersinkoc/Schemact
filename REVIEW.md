...
- **Recommendation:** Implement file size validation before reading migration files. Add a configurable maximum file size (e.g., 5MB) in the `SchemactConfig`. Use streaming parsers for files above a certain threshold, or reject files exceeding the limit with a clear error message. Consider adding a `maxMigrationFileSize` option to configuration.
...
- **Consequence:** Data corruption in the ledger file. Multiple concurrent migration processes could write to `.schemact_ledger.json` simultaneously, resulting in invalid JSON, lost migration records, or incorrect batch numbers. This violates the integrity guarantees that the ledger system is designed to provide.
...
- **Problem:** If database migrations succeed but ledger recording fails (line 132: `await this.ledger.recordBatch()`), the database changes are committed but not tracked in the ledger. This creates an inconsistent state where the database has been modified but Schemact believes the migrations haven't been applied yet. Subsequent `schemact up` commands will attempt to re-apply these migrations, causing duplicate key errors or other constraint violations.
...
- **File:** Multiple files
- **Details:** Some functions throw `SchemactError`, others throw `GeneratorError`, and some catch and re-throw native JavaScript errors. The error hierarchy isn't clearly documented, making it difficult for consumers of the library to implement proper error handling. Some error types extend `SchemactError`, but the inheritance chain isn't consistently used.
...
The `DbAdapter` interface provides clean separation between Schemact's core logic and database-specific implementations. This makes the system highly testable and extensible. However, consider formalizing this pattern with:
...
- Consider documenting that Schemact should not be run concurrently from multiple machines sharing a filesystem
...
❌ **No Input Sanitization for Raw SQL:** Lines prefixed with `>` in .sact files are passed directly to the database without any validation. While this is by design (escape hatch), it creates an XSS-like risk where malicious SQL could be injected into migration files.
...
10. **A10: SSRF** - ✅ N/A: No network requests made by Schemact itself
...
**Codebase:** Schemact v1.0.0 (Zero-Dependency Database Schema Management Tool)
